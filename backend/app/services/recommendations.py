    ...
    while remaining and len(ordered) < max_feed_items:
        ...
        for index, item in enumerate(remaining):
            ...
            if item.has_interaction:
                adjusted_score -= 8.0 if len(ordered) < settings.FEED_SIZE else 2.0
            ...
            if stage == 0:
                ...
                adjusted_score -= 0.35
            if is_exploration and len(ordered) < settings.FEED_SIZE:
                max_early_exploration = max(
                    1, int(settings.FEED_SIZE * settings.FEED_EXPLORATION_RATIO)
                )
            ...
            if explicit_interests and len(ordered) < settings.FEED_SIZE:
...
def persist_feed(
    ...
    *,
    ...
    for index, item in enumerate(ranked_articles, start=1):
